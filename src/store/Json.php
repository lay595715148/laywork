<?php
class Json extends Store implements Query {
    const JSON_FILE_SUFFIX = '.json';
    const JSON_SEQUENCE_NAME = 'sequences';
    ...
    public function connect() {
        $config = &$this->config;
        $link   = &$this->link;
        $host   = (array_key_exists(Config::KEY_STORE_JSON_HOST,$config))?$config[Config::KEY_STORE_JSON_HOST]:false;
        $name   = (array_key_exists(Config::KEY_STORE_JSON_NAME,$config))?$config[Config::KEY_STORE_JSON_NAME]:false;
        $dir    = (array_key_exists(Config::KEY_STORE_JSON_DIR,$config))?$config[Config::KEY_STORE_JSON_DIR]:false;
        ...
        if($name) {
            ...
            $link = $this->ls($path,Json::JSON_FILE_SUFFIX);
    ...
    public function choose($name) {
        ...
        if($host && $dir){
            $path = $layworkPath.$host.'/'.$dir.'/';
            $link = $this->load($path,$name,Json::JSON_FILE_SUFFIX);
        }
        return ($link)?true:false;
    }
    /**
     * @return array
     */
    public function ls($path, $mask = '.json') {
        $config  = &$this->config;
        $arrays  = array();
        ...
        while ($f = readdir($handler)) {
            ...
            if(file_exists($file)) {
                $arrays[$filename] = json_decode(File::read($file),true);
            }
        }
     ...
     * @return array
     */
    public function load($path,$name,$mask = '.json') {
        $config   = &$this->config;
        ...
        //$file     = $path.$prefix.$name.$suffix.$mask;
        ...
        if(is_array($name)) {
            foreach($name as $v) {
                ...
                if(file_exists($file)) { $return[$filename] = json_decode(File::read($file),true); }
            }
        }
        return $return;
    }
    ...
    public function select($name, $fields = '', $condition = '', $group = '', $order = '', $limit = '') {
        ...
        $link     = &$this->link;
        $result   = &$this->result;
        $host     = $config[Config::KEY_STORE_JSON_HOST];
        $dir      = $config[Config::KEY_STORE_JSON_DIR];
        $prefix   = ($config[Config::KEY_STORE_MEMORY_PREFIX])?$config[Config::KEY_STORE_MEMORY_PREFIX]:'';
        $suffix   = ($config[Config::KEY_STORE_MEMORY_SUFFIX])?$config[Config::KEY_STORE_MEMORY_SUFFIX]:'';
    ...
    public function insert($name,$fields = '',$values = '') {
        ...
        $basename = $prefix.$name.$suffix;
        $seq_name = $prefix.Json::JSON_SEQUENCE_NAME;
        $path     = $layworkPath.$host.'/'.$dir.'/';
        ...
        if(empty($clone)) {
            ...
            $result  = File::write(json_encode($content), $path, $basename, Json::JSON_FILE_SUFFIX);
            if($result && $increase) {
                File::write(json_encode($sequence), $path, $prefix.Json::JSON_SEQUENCE_NAME.$suffix, Json::JSON_FILE_SUFFIX);
